...
from BReg_NeXT import BREg_NeXT
...
data_loader = DataLoader(image_dataset, batch_size=10, shuffle=True)
...
loss_mean_list = []
print(model_mn)
number_params_model = np.sum(p.numel() for p in model_mn.parameters())
print("num of params: " + str(number_params_model))
model_mn.train()
for k in range(epoch):
    ...
    #     # #print(np.shape(heat_map))
        ...
        beta = torch.Tensor([0]).cuda()
        predict = model_mn.forward(batch_data, alpha, beta)
