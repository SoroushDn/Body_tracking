...
class ImageUtility:
    ...
    def random_rotate(self, _image, _label, file_name_image, file_name_points, fn):
        ...
        try:
            ...
            if not(min(landmark_arr_x) < 0.0 or min(landmark_arr_y) < 0.0 or max(landmark_arr_x) > 224 or max(landmark_arr_y) > 224):
                ...
                im.save(str(file_name_image) + '.jpg')

                pnt_file = open(str(file_name_points) + ".pts", "w")
                pre_txt = ["version: 1 \n", "n_points: 14 \n", "{ \n"]
    ...
    def augment(self, _image, _label):
        ...
        landmark_arr_xy, landmark_arr_x, landmark_arr_y = self.create_landmarks(_label, 1, 1)
        label = np.array(landmark_arr_x + landmark_arr_y).reshape([2, num_xy])
        marging = np.ones([1, num_xy])
        label = np.concatenate((label, marging), axis=0)

        label_t = np.dot(t_matrix, label)
        lbl_flat = np.delete(label_t, 2, axis=0).reshape([num_x_and_y])
    ...
    def random_augmentation(self, lbl, img):
        ...
        lbl = np.reshape(lbl, [num_x_and_y])
        return lbl, img
    ...
    def __rotate(self, img, landmark_old, degree, img_w, img_h):
        landmark_old = np.reshape(landmark_old, [num_xy, 2])
